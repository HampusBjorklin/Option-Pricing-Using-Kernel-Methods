...
e = zeros(length(anchor_list), 1);
e2 = zeros(length(anchor_list), 1);
domain = [0, 100];

parfor v = 1:length(anchor_list)
    tic
    anchor = anchor_list(v);
    %n = 10; 
    n = 30;
    N = 2*n + 1;
...
%     tmp(2:end) - tmp(1:end-1)
    ...
    e(v) = max(max(abs(true_val - sf)));
    e2(v) = norm(true_val - sf, inf);
    disp("Largest error for N = 10^" + num2str(log10(N)) + " is 10^" + num2str(log10(e(v))));
